...
impl Runtime {
    ...
    fn process_timing_event(&mut self, event: EventKind) {
        match event {
            ...
            EventKind::PpuScanline => {
                ...
                if vcount_match { self.dispstat |= DISPSTAT_VCOUNT; }
                else { self.dispstat &= !DISPSTAT_VCOUNT; }
    ...
    pub fn schedule_dma_completion(&mut self, channel: u8, cycles_from_now: u64) {
        ...
        self.scheduler.schedule_in(cycles_from_now, EventKind::DmaComplete { channel });
    }

    pub fn schedule_irq_sample(&mut self, cycles_from_now: u64) {
        self.scheduler.schedule_in(cycles_from_now, EventKind::IrqSample);
    }
    ...
    where F: FnMut(&mut Runtime, u32, bool) -> Result<(u32, bool), &'static str>,
    {
        ...
        loop {
            ...
            if self.power == PowerState::Halted {
                ...
                if self.power == PowerState::Halted { return Err("runtime is halted"); }
            }
