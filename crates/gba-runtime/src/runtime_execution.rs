...
impl Runtime {
    ...
    fn process_timing_event(&mut self, event: EventKind) {
        match event {
            EventKind::PpuHBlankStart => {
                ...
                if mode == 1 || mode == 2 {
                    self.ppu.render_affine_mode_scanline(self.dispcnt, self.vcount, &self.vram, &self.palette, &self.io);
                }
    ...
    pub fn dispatch_mode(&mut self, address: u32, thumb: bool) -> ! {
        self.cpu.set_thumb(thumb); self.cpu.r[REG_PC] = address & if thumb { !1 } else { !3 };
...
mod timing_tests {
    use super::*;
    use crate::bios::{IRQ_DMA0, IRQ_HBLANK, IRQ_TIMER0, IRQ_TIMER1, IRQ_VBLANK};
