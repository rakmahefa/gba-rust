...
fn arm_data_processing(rt: &mut Runtime, raw: u32) -> Option<(u32, bool)> {
    ...
    let mut result = None;
    match opcode {
        0 => { let v = lhs & rhs; if rd == REG_PC { rt.write_reg(rd, v & !3); result = Some((v & !3, false)); } else { rt.write_reg(rd, v); if set_flags { set_logic_flags(rt, v, sh_carry); } } }
        1 => { let v = lhs ^ rhs; if rd == REG_PC { rt.write_reg(rd, v & !3); result = Some((v & !3, false)); } else { rt.write_reg(rd, v); if set_flags { set_logic_flags(rt, v, sh_carry); } } }
        2 => { let (v, f) = arm7tdmi::sub_with_borrow(lhs, rhs, false); if rd == REG_PC { rt.write_reg(rd, v & !3); if set_flags { result = rt.exception_return(v); } else { result = Some((v & !3, false)); } } else { rt.write_reg(rd, v); if set_flags { rt.set_flags(f); } } }
        3 => { let (v, f) = arm7tdmi::sub_with_borrow(rhs, lhs, false); if rd == REG_PC { rt.write_reg(rd, v & !3); if set_flags { result = rt.exception_return(v); } else { result = Some((v & !3, false)); } } else { rt.write_reg(rd, v); if set_flags { rt.set_flags(f); } } }
        4 => { let (v, f) = arm7tdmi::add_with_carry(lhs, rhs, false); if rd == REG_PC { rt.write_reg(rd, v & !3); result = Some((v & !3, false)); } else { rt.write_reg(rd, v); if set_flags { rt.set_flags(f); } } }
        5 => { let (v, f) = arm7tdmi::add_with_carry(lhs, rhs, rt.nzcv().c); if rd == REG_PC { rt.write_reg(rd, v & !3); result = Some((v & !3, false)); } else { rt.write_reg(rd, v); if set_flags { rt.set_flags(f); } } }
        6 => { let (v, f) = arm7tdmi::sub_with_borrow(lhs, rhs, !rt.nzcv().c); if rd == REG_PC { rt.write_reg(rd, v & !3); if set_flags { result = rt.exception_return(v); } else { result = Some((v & !3, false)); } } else { rt.write_reg(rd, v); if set_flags { rt.set_flags(f); } } }
        7 => { let (v, f) = arm7tdmi::sub_with_borrow(rhs, lhs, !rt.nzcv().c); if rd == REG_PC { rt.write_reg(rd, v & !3); if set_flags { result = rt.exception_return(v); } else { result = Some((v & !3, false)); } } else { rt.write_reg(rd, v); if set_flags { rt.set_flags(f); } } }
        ...
        12 => { let v = lhs | rhs; if rd == REG_PC { rt.write_reg(rd, v & !3); result = Some((v & !3, false)); } else { rt.write_reg(rd, v); if set_flags { set_logic_flags(rt, v, sh_carry); } } }
        13 => { if rd == REG_PC { let v = rhs & !3; rt.write_reg(rd, v); if set_flags { result = rt.exception_return(rhs); } else { result = Some((v, false)); } } else { rt.write_reg(rd, rhs); if set_flags { set_logic_flags(rt, rhs, sh_carry); } } }
        14 => { let v = lhs & !rhs; if rd == REG_PC { rt.write_reg(rd, v & !3); result = Some((v & !3, false)); } else { rt.write_reg(rd, v); if set_flags { set_logic_flags(rt, v, sh_carry); } } }
        15 => { let v = !rhs; if rd == REG_PC { let target = v & !3; rt.write_reg(rd, target); if set_flags { result = rt.exception_return(v); } else { result = Some((target, false)); } } else { rt.write_reg(rd, v); if set_flags { set_logic_flags(rt, v, sh_carry); } } }
        _ => unreachable!(),
    }
    result
}
...
impl Runtime {
    pub fn execute_arm_instruction(&mut self, raw: u32) -> Option<(u32, bool)> {
        ...
        if raw & 0x0fff_fff0 == 0x012f_ff10 || raw & 0x0fff_fff0 == 0x012f_ff30 { return Some(self.dispatch_exchange(self.read_reg((raw & 0xf) as usize))); }
        if raw & 0x0e00_0000 == 0x0a00_0000 {
            ...
            if link { self.write_reg(REG_LR, (self.read_reg(REG_PC).wrapping_sub(4)) | 0); }
            self.write_reg(REG_PC, target & !3);
            return Some((target & !3, false));
        }
        ...
        if raw & 0x0fbf_0fff == 0x010f_0000 {
            let rd = ((raw >> 12) & 0xf) as usize;
            let value = if raw & (1 << 22) != 0 { self.cpu.spsr().unwrap_or(0) } else { self.cpu.cpsr };
        ...
        if raw & 0x0db0_f000 == 0x0120_f000 {
            ...
            if spsr { self.cpu.set_spsr(value); }
            else {
                let mode = self.mode();
                let allowed = mode.privileged();
                if allowed {
                    let mut cpsr = self.cpu.cpsr;
                    if field_mask & 1 != 0 { cpsr = (cpsr & !0xff) | (value & 0xff); }
                    if field_mask & 2 != 0 { cpsr = (cpsr & !0xff00) | (value & 0xff00); }
                    if field_mask & 4 != 0 { cpsr = (cpsr & !0xff0000) | (value & 0xff0000); }
                    if field_mask & 8 != 0 { cpsr = (cpsr & !0xff00_0000) | (value & 0xff00_0000); }
                    let new_mode = crate::CpuMode::from_cpsr(cpsr).unwrap_or(mode);
                    if new_mode != mode { self.cpu.switch_mode(new_mode); }
                    self.cpu.cpsr = cpsr;
