    ...
    match op {
        0 => {
            ...
            if rd == REG_PC {
                ...
                rt.write_reg(rd, v);
                if s {
                    set_logic_flags(rt, v, shift_carry);
                }
            }
        }
        1 => {
        ...
        13 => {
            if rd == REG_PC {
                ...
                rt.write_reg(rd, rhs);
                if s {
                    set_logic_flags(rt, rhs, shift_carry);
                }
            }
        }
        14 => {
...
fn arm_single(rt: &mut Runtime, raw: u32) -> Option<(u32, bool)> {
    ...
    let rd = ((raw >> 12) & 15) as usize;
    let base = rt.read_reg(rn);
    let off = if raw & (1 << 25) == 0 {
        raw & 0xfff
    } else {
        arm_operand2(rt, raw).0
    };
    let addr = if pre {
        if up {
            base.wrapping_add(off)
...
impl Runtime {
    pub fn execute_arm_instruction(&mut self, raw: u32) -> Option<(u32, bool)> {
        ...
        if !arm7tdmi::condition_holds(self.cpu.cpsr, condition(raw)) {
            return None;
        }
        ...
        if raw & 0x0e00_0000 == 0x0a00_0000 {
            ...
            if raw & (1 << 24) != 0 {
                self.write_reg(REG_LR, base.wrapping_sub(4));
            }
        ...
        if raw & 0x0f80_00f0 == 0x0080_0090 {
            ...
            let mut x = if signed {
                (self.read_reg(rm) as i32 as i64).wrapping_mul(self.read_reg(rs) as i32 as i64)
                    as u64
            } else {
            ...
            if raw & (1 << 21) != 0 {
                x = x.wrapping_add(
                    (u64::from(self.read_reg(hi)) << 32) | u64::from(self.read_reg(lo)),
                );
            }
            ...
            if raw & (1 << 20) != 0 {
                let o = self.nzcv();
                self.set_flags(arm7tdmi::Nzcv::new(x >> 63 != 0, x == 0, o.c, o.v));
            }
            return None;
        }
        if raw & 0x0fc0_00f0 == 0x0000_0090 {
            ...
            if raw & (1 << 21) != 0 {
                x = x.wrapping_add(self.read_reg(rn));
            }
            self.write_reg(rd, x);
            if raw & (1 << 20) != 0 {
                let o = self.nzcv();
                self.set_flags(arm7tdmi::Nzcv::new(x & 0x8000_0000 != 0, x == 0, o.c, o.v));
            }
            return None;
        }
        ...
        if raw & 0x0e00_0090 == 0x0000_0090 {
            return arm_halfword(self, raw);
        }
        if raw & 0x0e00_0000 == 0x0800_0000 {
            return arm_block(self, raw);
        }
        if raw & 0x0c00_0000 == 0x0400_0000 {
            return arm_single(self, raw);
        }
        ...
        if raw & 0x0db0_f000 == 0x0120_f000 {
            ...
            if spsr {
                ...
                if mode.privileged() {
                    ...
                    if mask & 2 != 0 {
                        c = (c & !0xff00) | (value & 0xff00);
                    }
                    if mask & 4 != 0 {
                        c = (c & !0xff0000) | (value & 0xff0000);
                    }
                    if mask & 8 != 0 {
                        c = (c & !0xff00_0000) | (value & 0xff00_0000);
                    }
                    let nm = crate::CpuMode::from_cpsr(c).unwrap_or(mode);
                    if nm != mode {
                        self.cpu.switch_mode(nm);
                    }
                    self.cpu.cpsr = c;
        ...
        if raw & 0x0c00_0000 == 0 {
            return arm_data_processing(self, raw);
        }
    ...
    pub fn execute_thumb_instruction(&mut self, raw: u16) -> Option<(u32, bool)> {
        ...
        if raw & 0xf800 == 0x1800 {
            ...
            let rhs = if imm {
                ...
                self.read_reg(((raw >> 6) & 7) as usize)
            };
            if sub {
                self.sub(rd, self.read_reg(rs), rhs, true)
            } else {
                self.add(rd, self.read_reg(rs), rhs, true)
            }
            return None;
        }
        ...
        if raw & 0xf800 == 0x3000 || raw & 0xf800 == 0x3800 {
            ...
            let v = (raw & 255) as u32;
            if sub {
                self.sub(rd, self.read_reg(rd), v, true)
            } else {
                self.add(rd, self.read_reg(rd), v, true)
            }
            return None;
        }
        if raw & 0xfc00 == 0x4000 {
            ...
            match op {
                ...
                2 => {
                    ...
                    self.write_reg(rd, r.value);
                    if n != 0 {
                        set_logic_flags(self, r.value, Some(r.carry))
                    } else {
                        set_logic_flags(self, r.value, None)
                    }
                }
                3 => {
                    let n = (b & 255) as u8;
                ...
                9 => {
                    let (v, f) = arm7tdmi::sub_with_borrow(0, b, false);
                    self.write_reg(rd, v);
                    self.set_flags(f)
                }
                10 => self.compare(a, b),
                11 => {
                    let (_, f) = arm7tdmi::add_with_carry(a, b, false);
                    self.set_flags(f)
                }
                12 => {
                    let v = a | b;
        ...
        if raw & 0xf000 == 0x5000 {
            ...
            match op {
                ...
                5 => {
                    ...
                    self.write_reg(rd, if v & 0x80 != 0 { v | 0xffff_ff00 } else { v })
                }
                6 => {
                    let v = self.read16(a) as u32;
                    self.write_reg(rd, if v & 0x8000 != 0 { v | 0xffff_0000 } else { v })
                }
        ...
        if raw & 0xe000 == 0x6000 {
            ...
            if load {
                ...
                self.write8(a, self.read_reg(rd) as u8)
            } else {
                self.write32(a & !3, self.read_reg(rd))
            }
            return None;
        }
        if raw & 0xf000 == 0x8000 {
            ...
            if load {
                self.write_reg(rd, self.read16(a) as u32)
            } else {
                self.write16(a, self.read_reg(rd) as u16)
            }
            return None;
        }
        ...
        if raw & 0xfe00 == 0xb400 || raw & 0xfe00 == 0xbc00 {
            ...
            if load {
                ...
                for r in 0..8usize {
                    if regs & (1 << r) != 0 {
                        self.write_reg(r, self.read32(a));
                        a = a.wrapping_add(4);
                    }
                }
                if extra {
                ...
                let sp = self
                    .read_reg(13)
                    .wrapping_sub((regs.count_ones() + u32::from(extra)) * 4);
                let mut a = sp;
                for r in 0..8usize {
                    if regs & (1 << r) != 0 {
                        self.write32(a & !3, self.read_reg(r));
                        a = a.wrapping_add(4);
                    }
                }
                if extra {
                    self.write32(a & !3, self.read_reg(REG_LR));
                }
