...
pub enum DmaTrigger { Immediate, VBlank, HBlank, Special }
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum DmaAddressMode { Increment, Decrement, Fixed, Reload }

impl DmaAddressMode {
    fn from_control(value: u16) -> Self {
        match value & 0b11 { 0 => Self::Increment, 1 => Self::Decrement, 2 => Self::Fixed, _ => Self::Reload }
    }
...
pub struct DmaChannel {
    pub source: u32,
    pub destination: u32,
    /// Programmer-visible transfer count register.
    pub count: u16,
    pub control: u16,
    current_source: u32,
    current_destination: u32,
    reload_destination: u32,
    /// Internal transfer progress. This must never be exposed as CNT_L.
    remaining: u32,
    start_pending: bool,
}
...
impl DmaChannel {
    ...
    pub fn transfer_count(&self, channel: usize) -> u32 {
        if self.count == 0 { if channel == 3 { 0x1_0000 } else { 0x4000 } } else { u32::from(self.count) }
    }
    ...
    pub fn finish(&mut self) {
        ...
        self.remaining = 0;
        let destination_mode = DmaAddressMode::from_control((self.control & CONTROL_DEST_MASK) >> 5);
        if matches!(destination_mode, DmaAddressMode::Reload) { self.destination = self.reload_destination; }
    }
    ...
    pub fn write_control(&mut self, value: u16, channel: usize) {
        ...
        if !was_enabled && self.enabled() && matches!(self.trigger(), DmaTrigger::Immediate) { self.start_pending = true; }
        if !self.enabled() {
            self.start_pending = false;
            self.remaining = 0;
        }
...
impl Default for DmaController {
    fn default() -> Self { Self { channels: std::array::from_fn(|_| DmaChannel::default()), active: None, busy_until: 0 } }
}
...
impl DmaController {
    ...
    pub fn request_trigger(&mut self, trigger: DmaTrigger) {
        for channel in &mut self.channels { if channel.enabled() && channel.trigger() == trigger { channel.request(); } }
    }
    ...
    pub fn begin_selected(&mut self, now: u64, waitcnt: u16) -> Option<DmaTransfer> {
        ...
        self.channels[channel].begin(channel);
        let state = self.channels[channel];
        let count = state.remaining();
        ...
        Some(DmaTransfer { channel, source: state.current_source, destination: state.current_destination, count, width, cycles: cycles.max(1), irq: state.control & CONTROL_IRQ != 0 })
    }
...
fn rom_bank(address: u32) -> u8 { match address { bus::ROM0_START..=bus::ROM0_END => 0, bus::ROM1_START..=bus::ROM1_END => 1, _ => 2 } }

fn rom_initial_wait(waitcnt: u16, bank: u8) -> u32 {
    let bits = match bank { 0 => (waitcnt >> 2) & 0x3, 1 => (waitcnt >> 5) & 0x3, _ => (waitcnt >> 8) & 0x3 };
    match bits { 0 => 4, 1 => 3, 2 => 2, _ => 8 }
}

fn rom_sequential_wait(waitcnt: u16, bank: u8) -> u32 {
    match bank { 0 => if waitcnt & 0x10 != 0 { 1 } else { 2 }, 1 => if waitcnt & 0x80 != 0 { 1 } else { 4 }, _ => if waitcnt & 0x400 != 0 { 1 } else { 8 } }
}
...
fn internal_rate(source: BusRegion, destination: BusRegion, width: u32) -> (u32, u32) {
    match (source, destination, width) {
        (BusRegion::Ewram, BusRegion::Ewram, 2) => (4, 2), (BusRegion::Ewram, BusRegion::Ewram, 4) => (8, 4),
        (BusRegion::Ewram, BusRegion::Iwram, 2) => (4, 2), (BusRegion::Ewram, BusRegion::Iwram, 4) => (7, 4),
        (BusRegion::Iwram, BusRegion::Ewram, 2) => (4, 2), (BusRegion::Iwram, BusRegion::Ewram, 4) => (7, 3),
        (BusRegion::Iwram, BusRegion::Iwram, 2) => (2, 0), (BusRegion::Iwram, BusRegion::Iwram, 4) => (2, 0),
        (BusRegion::Iwram, BusRegion::Io, 2) => (2, 0), (BusRegion::Iwram, BusRegion::Io, 4) => (2, 0),
        (BusRegion::Iwram, BusRegion::Palette, 2) => (2, 1), (BusRegion::Iwram, BusRegion::Palette, 4) => (2, 1),
        (BusRegion::Iwram, BusRegion::Vram, 2) => (2, 1), (BusRegion::Iwram, BusRegion::Vram, 4) => (2, 1),
        (BusRegion::Iwram, BusRegion::Oam, 2) => (2, 0), (BusRegion::Iwram, BusRegion::Oam, 4) => (2, 0),
        (BusRegion::Ewram, BusRegion::Io, 2) => (6, 1), (BusRegion::Ewram, BusRegion::Io, 4) => (12, 2),
        (BusRegion::Ewram, BusRegion::Palette, 2) => (6, 2), (BusRegion::Ewram, BusRegion::Palette, 4) => (12, 4),
        (BusRegion::Ewram, BusRegion::Vram, 2) => (6, 2), (BusRegion::Ewram, BusRegion::Vram, 4) => (12, 4),
        (BusRegion::Ewram, BusRegion::Oam, 2) => (6, 1), (BusRegion::Ewram, BusRegion::Oam, 4) => (12, 2),
        _ => match width { 4 => (7, 1), _ => (4, 1) },
    }
}
...
mod tests {
    ...
    #[test]
    fn zero_count_expands_to_architectural_count_without_mutating_register() {
        let mut controller = DmaController::default();
        ...
        assert_eq!(controller.channels[0].count, 0);
        assert_eq!(controller.channels[3].count, 0);
    }

    #[test]
    fn begin_tracks_expanded_length_in_execution_state_only() {
        let mut channel = DmaChannel { control: CONTROL_ENABLE, count: 0, ..Default::default() };
        channel.begin(0);
    ...
    fn control_masks_keep_dma3_bit_15_configuration_valid() {
        let channel = DmaChannel { control: 0xffff, ..Default::default() };
        assert_eq!(channel.control, 0xffff);
        assert_eq!(channel.control & DMA3_CONTROL_MASK, DMA3_CONTROL_MASK);
    }

    #[test]
    fn rom_waitstate_changes_transfer_cost() {
        let fast = transfer_cycles(bus::ROM0_START, bus::EWRAM_START, 4, 2, 0x0010);
