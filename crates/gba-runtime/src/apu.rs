...
impl Default for Fifo {
    fn default() -> Self {
        Self { data: [0; FIFO_CAPACITY], read: 0, write: 0, len: 0 }
    }
}
...
impl ApuChannel {
    fn new(kind: PsgKind) -> Self {
        Self { enabled: false, frequency: 0, volume: 0, duty: if matches!(kind, PsgKind::Square1 | PsgKind::Square2) { 2 } else { 0 }, length_counter: 0, envelope_period: 0, envelope_increase: false, envelope_timer: 0, sweep_period: 0, sweep_shift: 0, sweep_negate: false, phase: 0, lfsr: 0x7fff }
    }

    fn frame_length_tick(&mut self) {
    ...
    fn frame_envelope_tick(&mut self) {
        ...
        if self.envelope_timer == 0 {
            ...
            if self.envelope_increase { self.volume = self.volume.saturating_add(1).min(15); }
            else { self.volume = self.volume.saturating_sub(1); }
        }
    }
    ...
    fn sample(&mut self, kind: PsgKind) -> i16 {
        ...
        match kind {
            PsgKind::Square1 | PsgKind::Square2 => {
                let duty_threshold = match self.duty & 3 { 0 => 32, 1 => 64, 2 => 128, _ => 192 };
                let value = if (self.phase & 0xff) < duty_threshold { 1 } else { -1 };
                self.phase = self.phase.wrapping_add((2048u16.saturating_sub(self.frequency)).max(1));
                value * i16::from(self.volume) * 256
            }
            PsgKind::Wave => {
                const WAVE: [i16; 16] = [1, 3, 5, 7, 7, 5, 3, 1, -1, -3, -5, -7, -7, -5, -3, -1];
                let value = WAVE[((self.phase >> 8) & 15) as usize];
                self.phase = self.phase.wrapping_add((2048u16.saturating_sub(self.frequency)).max(1));
                value * i16::from(self.volume.saturating_mul(2)) * 32
            }
            PsgKind::Noise => {
                let bit = ((self.lfsr ^ (self.lfsr >> 1)) & 1) != 0;
                self.lfsr = (self.lfsr >> 1) | if bit { 0x4000 } else { 0 };
                if bit { i16::from(self.volume) * 256 } else { -(i16::from(self.volume) * 256) }
            }
        }
    }
}
...
pub struct Apu {
    ...
    pub frame_step_accumulated: u64,
    pub frame_sequence_step: u8,
    pub soundcnt_l: u16,
    pub soundcnt_h: u16,
    pub soundcnt_x: u16,
    pub soundbias: u16,
    ...
    fifo_a_refill_pending: bool,
    fifo_b_refill_pending: bool,
    fifo_a: Fifo,
    fifo_b: Fifo,
}
...
impl Apu {
    pub fn tick(&mut self, samples: u64) {
        ...
        for _ in 0..samples { let _ = self.mix_sample(); }
    }

    pub fn advance_cycles(&mut self, cycles: u64) -> u64 {
        self.cycles_accumulated = self.cycles_accumulated.saturating_add(cycles);
        self.frame_step_accumulated = self.frame_step_accumulated.saturating_add(cycles);
        let frame_steps = self.frame_step_accumulated / CYCLES_PER_FRAME_STEP;
        self.frame_step_accumulated %= CYCLES_PER_FRAME_STEP;
        for _ in 0..frame_steps { self.step_frame_sequence(); }
    ...
    fn step_frame_sequence(&mut self) {
        ...
        if step % 2 == 0 { for channel in &mut self.channel { channel.frame_length_tick(); } }
    ...
    pub fn on_timer_overflow(&mut self, timer_index: usize) {
        ...
        if a_enabled && timer_index == timer_a {
            if self.fifo_a.pop().is_some() {
                self.fifo_a_samples += 1;
                self.fifo_a_refill_pending |= self.fifo_a.len() <= FIFO_REFILL_THRESHOLD;
            } else { self.fifo_a_underruns += 1; self.fifo_a_refill_pending = true; }
        }
        if b_enabled && timer_index == timer_b {
            if self.fifo_b.pop().is_some() {
                self.fifo_b_samples += 1;
                self.fifo_b_refill_pending |= self.fifo_b.len() <= FIFO_REFILL_THRESHOLD;
            } else { self.fifo_b_underruns += 1; self.fifo_b_refill_pending = true; }
        }
    }

    pub fn reset_fifos(&mut self, fifo_a: bool, fifo_b: bool) { if fifo_a { self.fifo_a.clear(); self.fifo_a_refill_pending = false; } if fifo_b { self.fifo_b.clear(); self.fifo_b_refill_pending = false; } }

    pub fn configure_square(&mut self, channel: usize, frequency: u16, duty: u8, length: u8, volume: u8, envelope_period: u8, envelope_increase: bool) {
        if let Some(channel) = self.channel.get_mut(channel.min(1)) { channel.enabled = true; channel.frequency = frequency.min(2047); channel.duty = duty & 3; channel.length_counter = length; channel.volume = volume.min(15); channel.envelope_period = envelope_period & 7; channel.envelope_timer = channel.envelope_period; channel.envelope_increase = envelope_increase; }
    }

    pub fn configure_wave(&mut self, frequency: u16, length: u8, volume: u8) {
        let channel = &mut self.channel[2]; channel.enabled = true; channel.frequency = frequency.min(2047); channel.length_counter = length; channel.volume = volume.min(15);
    }

    pub fn configure_noise(&mut self, length: u8, volume: u8, envelope_period: u8, envelope_increase: bool) {
        let channel = &mut self.channel[3]; channel.enabled = true; channel.length_counter = length; channel.volume = volume.min(15); channel.envelope_period = envelope_period & 7; channel.envelope_timer = channel.envelope_period; channel.envelope_increase = envelope_increase; channel.lfsr = 0x7fff;
    }

    fn mix_sample(&mut self) -> i16 {
        if self.soundcnt_x & 0x0080 == 0 { return 0; }
        let mut psg = 0i32;
        psg += i32::from(self.channel[0].sample(PsgKind::Square1));
        psg += i32::from(self.channel[1].sample(PsgKind::Square2));
        psg += i32::from(self.channel[2].sample(PsgKind::Wave));
        psg += i32::from(self.channel[3].sample(PsgKind::Noise));
        let direct_a = if self.soundcnt_h & 0x0004 != 0 { self.fifo_a.pop().map(i32::from).unwrap_or(0) * 256 } else { 0 };
        let direct_b = if self.soundcnt_h & 0x0008 != 0 { self.fifo_b.pop().map(i32::from).unwrap_or(0) * 256 } else { 0 };
        let master = ((self.soundcnt_l & 0x0007) + ((self.soundcnt_l >> 4) & 0x0007)) as i32 + 2;
        let total = (psg * master / 16) + direct_a + direct_b;
        self.mixed_samples = self.mixed_samples.wrapping_add(1);
...
mod tests {
    ...
    #[test] fn frame_sequencer_runs_at_512_hz() { let mut apu = Apu::default(); assert_eq!(apu.advance_cycles(CYCLES_PER_FRAME_STEP - 1), 0); assert_eq!(apu.frame_sequence_step, 0); apu.advance_cycles(1); assert_eq!(apu.frame_sequence_step, 1); }
    ...
    #[test] fn square_channel_can_produce_mixed_samples() { let mut apu = Apu::default(); apu.soundcnt_x = 0x0080; apu.configure_square(0, 512, 2, 10, 8, 0, false); apu.advance_cycles(CYCLES_PER_SAMPLE); assert!(apu.mixed_sample_count() >= 1); }
}