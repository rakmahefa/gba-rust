...
pub struct ApuChannel { pub enabled: bool, pub frequency: u16, pub volume: u8, pub duty: u8, pub length_counter: u8, pub envelope_period: u8, pub envelope_increase: bool, pub envelope_timer: u8, pub sweep_period: u8, pub sweep_shift: u8, pub sweep_negate: bool, pub phase: u16, pub lfsr: u16 }
impl ApuChannel {
...
impl Apu {
    ...
    pub fn on_timer_overflow(&mut self, timer_index: usize) { let timer_a = usize::from((self.soundcnt_h >> 10) & 1); let timer_b = usize::from((self.soundcnt_h >> 14) & 1); let a_enabled = self.soundcnt_h & 0x0300 != 0; let b_enabled = self.soundcnt_h & 0x3000 != 0; if a_enabled && timer_index == timer_a { if self.fifo_a.pop().is_some() { self.fifo_a_samples += 1; self.fifo_a_refill_pending |= self.fifo_a.len() <= FIFO_REFILL_THRESHOLD; } else { self.fifo_a_underruns += 1; self.fifo_a_refill_pending = true; } } if b_enabled && timer_index == timer_b { if self.fifo_b.pop().is_some() { self.fifo_b_samples += 1; self.fifo_b_refill_pending |= self.fifo_b.len() <= FIFO_REFILL_THRESHOLD; } else { self.fifo_b_underruns += 1; self.fifo_b_refill_pending = true; } } }
    ...
    pub fn configure_square(&mut self, channel: usize, frequency: u16, duty: u8, length: u8, volume: u8, envelope_period: u8, envelope_increase: bool) { if let Some(channel) = self.channel.get_mut(channel.min(1)) { channel.enabled = true; channel.frequency = frequency.min(2047); channel.duty = duty & 3; channel.length_counter = length; channel.volume = volume.min(15); channel.envelope_period = envelope_period & 7; channel.envelope_timer = channel.envelope_period; channel.envelope_increase = envelope_increase; } }
