...
impl Cpu {
    ...
    pub fn set_nzcv_masked(&mut self, flags: Nzcv, mask: u8) {
        if mask & 1 != 0 { if flags.n { self.cpsr |= CPSR_N } else { self.cpsr &= !CPSR_N } }
        if mask & 2 != 0 { if flags.z { self.cpsr |= CPSR_Z } else { self.cpsr &= !CPSR_Z } }
        if mask & 4 != 0 { if flags.c { self.cpsr |= CPSR_C } else { self.cpsr &= !CPSR_C } }
        if mask & 8 != 0 { if flags.v { self.cpsr |= CPSR_V } else { self.cpsr &= !CPSR_V } }
    }
...
impl ExceptionKind {
    pub const fn mode(self) -> CpuMode { match self { Self::Undefined => CpuMode::Undefined, Self::SoftwareInterrupt => CpuMode::Supervisor, Self::PrefetchAbort | Self::DataAbort => CpuMode::Abort, Self::Irq => CpuMode::Irq, Self::Fiq => CpuMode::Fiq } }
    pub const fn vector(self) -> u32 { match self { Self::Undefined => 0x04, Self::SoftwareInterrupt => 0x08, Self::PrefetchAbort => 0x0c, Self::DataAbort => 0x10, Self::Irq => 0x18, Self::Fiq => 0x1c } }
...
impl Runtime {
    ...
    pub fn mode(&self) -> CpuMode { self.cpu.mode() }
    pub fn set_thumb(&mut self, thumb: bool) { self.cpu.set_thumb(thumb); }
    pub fn switch_mode(&mut self, mode: CpuMode) { self.cpu.switch_mode(mode); }
