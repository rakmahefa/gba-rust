...
impl Runtime {
    ...
    pub fn read8(&self, address: u32) -> u8 {
        if (0x0800_0000..0x0E00_0000).contains(&address) { self.cartridge.as_ref().and_then(|c| c.rom.get((address - 0x0800_0000) as usize)).copied().unwrap_or(0xff) }
        else if (0x0E00_0000..=0x0E00_FFFF).contains(&address) { self.cartridge.as_ref().map(|c| c.save.read((address - 0x0E00_0000) as usize)).unwrap_or(0xff) }
...
mod tests {
    use super::*;
    #[test]
    fn compare_updates_all_nzcv_flags() { let mut runtime = Runtime::new(); runtime.compare(0x7fff_ffff, u32::MAX); assert!(runtime.cpu.cpsr & CPSR_N != 0); assert!(runtime.cpu.cpsr & CPSR_C == 0); assert!(runtime.cpu.cpsr & CPSR_V != 0); }
    #[test]
    ...
    #[test]
    fn mov_preserves_c_and_v_when_setting_logical_flags() { let mut runtime = Runtime::new(); runtime.cpu.cpsr = CPSR_C | CPSR_V | CpuMode::System as u32; runtime.mov(0, 0, true); assert!(runtime.cpu.cpsr & CPSR_Z != 0); assert!(runtime.cpu.cpsr & CPSR_C != 0); assert!(runtime.cpu.cpsr & CPSR_V != 0); }
    #[test]
    fn adc_and_sbc_consume_the_current_carry_bit() { let mut runtime = Runtime::new(); runtime.cpu.cpsr = CPSR_C | CpuMode::System as u32; runtime.adc(0, 1, 1, true); assert_eq!(runtime.read_reg(0), 3); runtime.cpu.cpsr = CPSR_C | CpuMode::System as u32; runtime.sbc(1, 3, 1, true); assert_eq!(runtime.read_reg(1), 2); runtime.cpu.cpsr &= !CPSR_C; runtime.adc(2, 1, 1, true); assert_eq!(runtime.read_reg(2), 2); runtime.cpu.cpsr &= !CPSR_C; runtime.sbc(3, 3, 1, true); assert_eq!(runtime.read_reg(3), 1); }
    #[test]
    ...
    #[test]
    fn word_reads_apply_arm_unaligned_rotation() { let mut runtime = Runtime::new(); runtime.write32(0x0400_0000, 0x4433_2211); assert_eq!(runtime.read32(0x0400_0001), 0x1144_3322); assert_eq!(runtime.read32(0x0400_0002), 0x2211_4433); }
    #[test]
