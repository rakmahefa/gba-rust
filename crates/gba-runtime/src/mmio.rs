...
impl MmioAccess {
    #[inline] pub const fn can_read(self) -> bool { matches!(self, Self::ReadOnly | Self::ReadWrite) }
    #[inline] pub const fn can_write(self) -> bool { matches!(self, Self::ReadWrite | Self::WriteOnly) }
}
...
pub const fn register(address: u32) -> Option<MmioRegister> {
    match address {
        DISPCNT | DISPCNT_HI => Some(DISPCNT_REGISTER), DISPSTAT | DISPSTAT_HI => Some(DISPSTAT_REGISTER),
        VCOUNT | VCOUNT_HI => Some(VCOUNT_REGISTER), KEYINPUT | KEYINPUT_HI => Some(KEYINPUT_REGISTER),
        KEYCNT | KEYCNT_HI => Some(KEYCNT_REGISTER), IE | IE_HI => Some(IE_REGISTER), IF | IF_HI => Some(IF_REGISTER),
        WAITCNT | WAITCNT_HI => Some(WAITCNT_REGISTER), IME | IME_HI => Some(IME_REGISTER), POSTFLG => Some(POSTFLG_REGISTER),
        HALTCNT => Some(HALTCNT_REGISTER), _ => None,
    }
}
