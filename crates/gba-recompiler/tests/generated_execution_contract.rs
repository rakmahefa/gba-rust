...
fn generated_blocks_execute_across_edges_and_preserve_memory_state() {
    ...
    let result = runtime
        .run_generated_contract(ROM_BASE, false, Some(16), |rt, address, thumb| {
            ...
            matches!((address, thumb), (0x0800_0000, false) | (0x0800_0004, false) | (0x0800_0008, false))
        })
...
fn generated_return_to_linked_block_is_resumed_instead_of_terminating() {
    let mut runtime = Runtime::new();
    let result = runtime
        .run_generated_contract(ROM_BASE, false, Some(8), |rt, address, thumb| {
            match (address, thumb) {
                (0x0800_0000, false) => {
                    rt.write_reg(0, 7);
            ...
            matches!((address, thumb), (0x0800_0000, false) | (0x0800_0004, false))
        })
...
fn generated_execution_step_limit_is_deterministic_for_self_loop() {
    let mut runtime = Runtime::new();
    let result = runtime
        .run_generated_contract(ROM_BASE, false, Some(4), |_, address, thumb| {
