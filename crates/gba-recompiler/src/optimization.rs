...
fn propagated_rhs(value: &Value, constants: &HashMap<u8, u32>) -> (Value, bool) {
    match value {
        Value::Reg(reg) => constants.get(reg).copied().map(Value::Imm).map_or((Value::Reg(*reg), false), |v| (v, true)),
        Value::Imm(value) => (Value::Imm(*value), false),
    }
}

fn normalize_instruction(instruction: &IrInstruction, constants: &mut HashMap<u8, u32>, report: &mut OptimizationReport) -> Vec<IrOp> {
        ...
        let op = match op {
            ...
            IrOp::Mov { dst, src } => {
                if matches!(src, Value::Reg(reg) if *reg == *dst) {
                    ...
                    report.changes.push(OptimizationChange { address: instruction.address, kind: OptimizationKind::IdentityMove });
                    IrOp::Nop
                } else {
                    ...
                    match src { Value::Imm(value) => { constants.insert(*dst, value); }, Value::Reg(_) => { constants.remove(dst); } }
            ...
            IrOp::Add { dst, lhs, rhs } => {
                ...
                match rhs {
                    Value::Imm(value) if value == 0 && *dst == *lhs => {
                        constants.remove(dst);
                        report.changes.push(OptimizationChange { address: instruction.address, kind: OptimizationKind::AddZero });
                    ...
                    Value::Imm(rhs_value) => {
                        if let Some(lhs_value) = constants.get(lhs).copied() {
                            ...
                            report.changes.push(OptimizationChange { address: instruction.address, kind: OptimizationKind::ConstantFold });
                    ...
                    Value::Reg(reg) => { constants.remove(dst); IrOp::Add { dst: *dst, lhs: *lhs, rhs: Value::Reg(reg) } }
                }
            }
            IrOp::Sub { dst, lhs, rhs } => {
                ...
                match rhs {
                    Value::Imm(value) if value == 0 && *dst == *lhs => {
                        constants.remove(dst);
                        report.changes.push(OptimizationChange { address: instruction.address, kind: OptimizationKind::SubZero });
                        IrOp::Mov { dst: *dst, src: Value::Reg(*lhs) }
                    }
                    Value::Imm(rhs_value) => {
                        if let Some(lhs_value) = constants.get(lhs).copied() {
                    ...
                    Value::Reg(reg) => { constants.remove(dst); IrOp::Sub { dst: *dst, lhs: *lhs, rhs: Value::Reg(reg) } }
                }
            }
            IrOp::Load { dst, base, offset, byte } => { constants.remove(dst); IrOp::Load { dst: *dst, base: *base, offset: *offset, byte: *byte } }
            IrOp::Store { src, base, offset, byte } => IrOp::Store { src: *src, base: *base, offset: *offset, byte: *byte },
            IrOp::Cmp { lhs, rhs } => { constants.clear(); IrOp::Cmp { lhs: *lhs, rhs: rhs.clone() } }
            IrOp::Branch { target, condition, link } => { if *link { constants.clear(); } IrOp::Branch { target: *target, condition: *condition, link: *link } }
            IrOp::BranchExchange { register, link } => { constants.clear(); IrOp::BranchExchange { register: *register, link: *link } }
            IrOp::Nop => IrOp::Nop,
            IrOp::Unknown { address, raw, mode } => { constants.clear(); IrOp::Unknown { address: *address, raw: *raw, mode: *mode } }
        };
        normalized.push(op);
    }
    normalized
...
fn semantic_instruction_from_ir(instruction: &IrInstruction) -> SemanticInstruction {
    ...
    SemanticInstruction { address: instruction.address, size: instruction.size, ops: instruction.ops.clone(), reads, writes, memory, flags }
}
    ...
    for function in &mut optimized.functions {
        for block in &mut function.blocks {
            ...
            for instruction in &block.instructions {
                let source = IrInstruction { address: instruction.address, size: instruction.size, ops: instruction.ops.clone() };
                let ops = normalize_instruction(&source, &mut constants, &mut report);
                instructions.push(semantic_instruction_from_ir(&IrInstruction { address: source.address, size: source.size, ops }));
            }
...
mod tests {
    ...
    fn program(instructions: Vec<IrOp>) -> SemanticProgram {
        let block = SemanticBlock {
            id: BlockId(0), address: 0x0800_0000, mode: Mode::Arm,
    ...
    fn folds_local_constants() {
        let input = program(vec![IrOp::Mov { dst: 0, src: Value::Imm(4) }, IrOp::Add { dst: 0, lhs: 0, rhs: Value::Imm(3) }, IrOp::Sub { dst: 1, lhs: 0, rhs: Value::Imm(2) }]);
