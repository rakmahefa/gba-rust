...
pub use memory::{
    read_arm, read_arm_at, read_thumb, read_thumb_at, read_thumb_bl, read_thumb_bl_at, DecodeError,
    ROM_BASE,
};
pub use semantic::{
...
mod tests {
    ...
    fn decodes_arm_data_processing_families() {
        for raw in [
            0xE0000000, 0xE0200000, 0xE0400000, 0xE0600000, 0xE0800000, 0xE0A00000, 0xE0C00000,
            0xE0E00000, 0xE1100000, 0xE1300000, 0xE1500000, 0xE1700000, 0xE1800000, 0xE1A00000,
            0xE1C00000, 0xE1E00000,
        ] {
            assert!(
                !matches!(
                    decode_arm(ROM_BASE, raw).kind,
    ...
    fn decodes_arm_system_and_memory_families() {
        for raw in [
            0xE0000090, 0xE0800090, 0xE1000090, 0xE1400090, 0xE4000000, 0xE4800000, 0xE8000000,
            0xEF000000, 0xEC000000, 0xEE000000,
        ] {
            assert!(
                !matches!(
                    decode_arm(ROM_BASE, raw).kind,
    ...
    fn decodes_thumb_major_families() {
        for raw in [
            ...
            assert!(
                !matches!(
                    decode_thumb(ROM_BASE, raw).kind,
