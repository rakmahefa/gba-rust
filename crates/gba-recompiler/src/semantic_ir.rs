...
fn semantic_instruction(ir: &IrInstruction) -> SemanticInstruction {
    ...
    for op in &ir.ops {
        match op {
            ...
            IrOp::Load { dst, base, offset: _, byte } => {
                reads.push(*base);
                writes.push(*dst);
                memory = Some(MemoryEffect::Read { width: if *byte { MemoryWidth::Byte } else { MemoryWidth::Word }, base: *base });
            }
            IrOp::Store { src, base, offset: _, byte } => {
                reads.extend([*src, *base]);
                memory = Some(MemoryEffect::Write { width: if *byte { MemoryWidth::Byte } else { MemoryWidth::Word }, base: *base });
            }
            IrOp::Branch { condition, link, .. } => {
                flags.read = *condition != Condition::Al;
...
fn terminator(block: &SemanticBlock) -> SemanticTerminator {
    ...
    match op {
        ...
        IrOp::BranchExchange { register, link: true } => SemanticTerminator::IndirectCall { register: *register, mode: Mode::Arm },
        IrOp::BranchExchange { register, link: false } => SemanticTerminator::IndirectBranch { register: *register },
        IrOp::Unknown { .. } => SemanticTerminator::Unknown,
    ...
    for function in &semantic.functions {
        ...
        for block in &function.blocks {
            if block.id.0 >= program.cfg.blocks.len() {
                return Err(format!("function {} references invalid block {}", function.id.0, block.id.0));
            }
            ...
            for (source, semantic) in source.ir.iter().zip(&block.instructions) {
                if source.address != semantic.address || source.size != semantic.size {
                    return Err(format!("block {} instruction identity changed", block.id.0));
                }
            }
