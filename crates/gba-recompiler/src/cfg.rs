...
fn next_key(i: Instruction) -> BlockKey { BlockKey { address: i.address + i.size as u32, mode: i.mode } }
fn in_rom(rom: &[u8], address: u32) -> bool { address >= ROM_BASE && address - ROM_BASE < rom.len() as u32 }
...
fn instruction_successors(instruction: Instruction) -> Vec<BlockKey> {
    ...
    match instruction.kind {
        ...
        InstructionKind::Arm(ArmOp::BranchExchange { link: true, .. }) => vec![next],
...
fn discover_reachable(rom: &[u8], entry: BlockKey) -> Result<(Vec<BlockKey>, HashMap<BlockKey, DiscoveredInstruction>), DecodeError> {
    ...
    while let Some(key) = queue.pop_front() {
        ...
        for successor in &successors { if in_rom(rom, successor.address) && queued.insert(successor.clone()) { queue.push_back(successor.clone()); } }
...
fn collect_leaders(_order: &[BlockKey], discovered: &HashMap<BlockKey, DiscoveredInstruction>, entry: &BlockKey) -> Vec<BlockKey> {
    ...
    for node in discovered.values() {
        if !is_fallthrough(node) {
            for successor in &node.successors { if discovered.contains_key(successor) { leaders.insert(successor.clone()); } }
        }
    }
...
fn partition_blocks(_order: &[BlockKey], discovered: &HashMap<BlockKey, DiscoveredInstruction>, leaders: &[BlockKey]) -> (Vec<BasicBlock>, HashMap<BlockKey, BlockId>) {
    ...
    for leader in leaders {
        let id = BlockId(blocks.len()); ids.insert(leader.clone(), id);
        let mut instructions = Vec::new(); let mut cursor = leader.clone();
        loop {
            let Some(node) = discovered.get(&cursor) else { break; };
            instructions.push(node.instruction);
            if !is_fallthrough(node) { break; }
            let next = node.successors[0].clone(); if leader_set.contains(&next) { break; } cursor = next;
        }
    ...
    for block in &mut blocks {
        ...
        let key = BlockKey { address: last.address, mode: last.mode }; let Some(node) = discovered.get(&key) else { continue; };
        let mut successors = node.successors.iter().filter_map(|successor| ids.get(successor).copied()).collect::<Vec<_>>();
        successors.sort_unstable_by_key(|id| id.0); successors.dedup(); block.successors = successors;
    }
    (blocks, ids)
}

fn validate_cfg(cfg: &ControlFlowGraph, expected_instruction_count: usize) {
    assert!(!cfg.blocks.is_empty()); assert!(cfg.entry.0 < cfg.blocks.len());
    let mut seen = HashMap::<BlockKey, BlockId>::new(); let mut owners = HashMap::<BlockKey, BlockId>::new();
    for block in &cfg.blocks {
        assert_eq!(block.id, BlockId(seen.len())); assert!(seen.insert(block.key.clone(), block.id).is_none()); assert!(!block.instructions.is_empty()); assert_eq!(block.instructions.len(), block.ir.len());
        let first = block.instructions[0]; assert_eq!(block.key, BlockKey { address: first.address, mode: first.mode });
        for (index, instruction) in block.instructions.iter().enumerate() { let key = BlockKey { address: instruction.address, mode: instruction.mode }; assert!(owners.insert(key, block.id).is_none()); if let Some(next) = block.instructions.get(index + 1) { assert_eq!(instruction.address + instruction.size as u32, next.address); assert_eq!(instruction.mode, next.mode); } }
...
pub fn analyze(rom: &[u8], entry: u32, entry_mode: Mode) -> Result<Program, AnalysisError> {
    ...
    let entry_id = ids[&entry_key]; let cfg = ControlFlowGraph { entry: entry_id, blocks }; validate_cfg(&cfg, discovered.len());
...
mod tests {
    ...
    #[test] fn discovery_preserves_sequential_arm_tail() { let bytes = arm_rom(&[0xE3A0_0001, 0xE280_0001]); let entry = BlockKey { address: ROM_BASE, mode: Mode::Arm }; let (order, discovered) = discover_reachable(&bytes, entry).unwrap(); assert_eq!(order.len(), 2); assert_eq!(discovered.len(), 2); }
    #[test] fn sequential_arm_instructions_remain_in_one_block() { let bytes = arm_rom(&[0xE3A0_0001, 0xE280_0001]); let program = analyze(&bytes, ROM_BASE, Mode::Arm).unwrap(); assert_eq!(program.cfg.blocks.len(), 1); assert_eq!(program.cfg.blocks[0].instructions.len(), 2); }
    #[test] fn discovers_arm_branch_and_fallthrough() { let bytes = arm_rom(&[0xEA00_0000, 0xE1A0_0000, 0xE1A0_0000]); let program = analyze(&bytes, ROM_BASE, Mode::Arm).unwrap(); assert_eq!(program.cfg.blocks.len(), 2); assert_eq!(program.cfg.blocks[0].successors, vec![BlockId(1)]); }
    #[test] fn conditional_branch_splits_fallthrough_and_target() { let bytes = vec![0x00,0xD0,0xC0,0x46,0xC0,0x46,0xC0,0x46]; let program = analyze(&bytes, ROM_BASE, Mode::Thumb).unwrap(); assert_eq!(program.cfg.blocks.len(), 3); assert_eq!(program.cfg.blocks[0].successors.len(), 2); }
    #[test] fn backward_branch_splits_an_already_discovered_block() { let bytes = arm_rom(&[0xEAFF_FFFE,0xE1A0_0000]); let program = analyze(&bytes, ROM_BASE, Mode::Arm).unwrap(); assert_eq!(program.cfg.blocks.len(), 1); }
    #[test] fn conditional_backward_branch_does_not_overlap_blocks() { let bytes = vec![0x00,0xD0,0xC0,0x46,0xFC,0xD0,0xC0,0x46]; let program = analyze(&bytes, ROM_BASE, Mode::Thumb).unwrap(); let mut keys = HashSet::new(); for block in &program.cfg.blocks { for instruction in &block.instructions { assert!(keys.insert((instruction.address,instruction.mode))); } } }
    #[test] fn unknown_instruction_terminates_discovery() { let bytes = arm_rom(&[0xFFFFFFFF,0xE1A0_0000]); let program = analyze(&bytes, ROM_BASE, Mode::Arm).unwrap(); assert_eq!(program.cfg.blocks.len(),1); }
    #[test] fn thumb_bl_is_a_four_byte_call_with_fallthrough() { let bytes = vec![0x00,0xF0,0x00,0xF8,0xC0,0x46,0xC0,0x46]; let program = analyze(&bytes, ROM_BASE, Mode::Thumb).unwrap(); assert_eq!(program.cfg.blocks.len(),2); assert_eq!(program.cfg.blocks[0].instructions[0].size,4); }
    #[test] fn indirect_linked_call_splits_continuation_block() { let program = analyze(&arm_rom(&[0xE12F_FF31,0xE1A0_0000]), ROM_BASE, Mode::Arm).unwrap(); assert_eq!(program.cfg.blocks.len(),2); assert_eq!(program.cfg.blocks[0].successors, vec![BlockId(1)]); }
}
