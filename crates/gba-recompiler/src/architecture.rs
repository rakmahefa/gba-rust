...
pub fn shift_immediate(value: u32, kind: ShiftType, amount: u8, carry_in: bool) -> ShiftResult {
    match kind {
        ShiftType::Lsl => {
            ...
            if amount < 32 { return ShiftResult { value: value << amount, carry: Some(value & (1 << (32 - amount)) != 0) }; }
            if amount == 32 { return ShiftResult { value: 0, carry: Some(value & 1 != 0) }; }
            ShiftResult { value: 0, carry: Some(false) }
        }
        ShiftType::Lsr => {
            let amount = if amount == 0 { 32 } else { amount as u32 };
            if amount < 32 { ShiftResult { value: value >> amount, carry: Some(value & (1 << (amount - 1)) != 0) } }
            else if amount == 32 { ShiftResult { value: 0, carry: Some(value & 0x8000_0000 != 0) } }
            else { ShiftResult { value: 0, carry: Some(false) } }
        }
        ShiftType::Asr => {
            let amount = if amount == 0 { 32 } else { amount as u32 };
            if amount < 32 { ShiftResult { value: ((value as i32) >> amount) as u32, carry: Some(value & (1 << (amount - 1)) != 0) } }
...
pub fn shift_register(value: u32, kind: ShiftType, amount: u8, carry_in: bool) -> ShiftResult {
    match amount {
        0 => ShiftResult { value, carry: Some(carry_in) },
        1..=31 => shift_immediate(value, kind, amount, carry_in),
        32 => match kind {
            ShiftType::Lsl | ShiftType::Lsr => ShiftResult { value: 0, carry: Some(value & if matches!(kind, ShiftType::Lsl) { 1 } else { 0x8000_0000 } != 0) },
            ShiftType::Asr => ShiftResult { value: if value & 0x8000_0000 != 0 { u32::MAX } else { 0 }, carry: Some(value & 0x8000_0000 != 0) },
            ShiftType::Ror => ShiftResult { value, carry: Some(value & 0x8000_0000 != 0) },
        },
        _ => ShiftResult { value: 0, carry: Some(false) },
    }
}
