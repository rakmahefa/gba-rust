...
fn emit_exception_return_probe(out: &mut String, op: ArmExtended) -> bool {
    let ArmExtended::DataProcessing { op, rd: 15, rn, op2, set_flags: true } = op else {
        return false;
    };
    let (rhs, _) = structured_operand2(out, op2);
    let lhs = format!("rt.read_reg({rn})");
    let result = match op {
        ArmDataOp::And => format!("{lhs} & {rhs}"),
        ArmDataOp::Eor => format!("{lhs} ^ {rhs}"),
        ArmDataOp::Sub => format!("{lhs}.wrapping_sub({rhs})"),
        ArmDataOp::Rsb => format!("{rhs}.wrapping_sub({lhs})"),
        ArmDataOp::Add => format!("{lhs}.wrapping_add({rhs})"),
        ArmDataOp::Adc => format!("{lhs}.wrapping_add({rhs}).wrapping_add(rt.nzcv().c as u32)"),
        ArmDataOp::Sbc => format!("{lhs}.wrapping_sub({rhs}).wrapping_sub((!rt.nzcv().c) as u32)"),
        ArmDataOp::Rsc => format!("{rhs}.wrapping_sub({lhs}).wrapping_sub((!rt.nzcv().c) as u32)"),
        ArmDataOp::Orr => format!("{lhs} | {rhs}"),
        ArmDataOp::Mov => rhs,
        ArmDataOp::Bic => format!("{lhs} & !({rhs})"),
        ArmDataOp::Mvn => format!("!({rhs})"),
        ArmDataOp::Tst | ArmDataOp::Teq | ArmDataOp::Cmp | ArmDataOp::Cmn => return false,
    };
    let _ = writeln!(out, "    let exception_return_value = {result};");
    let _ = writeln!(out, "    if rt.mode().has_spsr() {{ if let Some((target, thumb)) = rt.return_from_exception(exception_return_value) {{ return Ok(GeneratedBlockExit::return_to(target, thumb)); }} return Err(\"generated exception return has no restorable SPSR\"); }}");
    true
}

pub fn emit_arm_extended(out: &mut String, op: ArmExtended) {
    let _ = emit_exception_return_probe(out, op);
    match op {
        ArmExtended::DataProcessing { op, rd, rn, op2, set_flags } => {
            let (rhs, carry) = structured_operand2(out, op2);
            let lhs = format!("rt.read_reg({rn})");
            match op {
                ArmDataOp::And => { let _ = writeln!(out, "    let value = {lhs} & {rhs}; rt.write_reg({rd}, value);"); if set_flags { emit_flags_from_logic(out, "value", &carry); } }
                ArmDataOp::Eor => { let _ = writeln!(out, "    let value = {lhs} ^ {rhs}; rt.write_reg({rd}, value);"); if set_flags { emit_flags_from_logic(out, "value", &carry); } }
                ArmDataOp::Orr => { let _ = writeln!(out, "    let value = {lhs} | {rhs}; rt.write_reg({rd}, value);"); if set_flags { emit_flags_from_logic(out, "value", &carry); } }
                ArmDataOp::Bic => { let _ = writeln!(out, "    let value = {lhs} & !{rhs}; rt.write_reg({rd}, value);"); if set_flags { emit_flags_from_logic(out, "value", &carry); } }
                ArmDataOp::Mvn => { let _ = writeln!(out, "    let value = !{rhs}; rt.write_reg({rd}, value);"); if set_flags { emit_flags_from_logic(out, "value", &carry); } }
                ArmDataOp::Mov => { let _ = writeln!(out, "    rt.write_reg({rd}, {rhs});"); if set_flags { emit_flags_from_logic(out, &rhs, &carry); } }
                ArmDataOp::Tst => { let _ = writeln!(out, "    let value = {lhs} & {rhs};"); emit_flags_from_logic(out, "value", &carry); }
                ArmDataOp::Teq => { let _ = writeln!(out, "    let value = {lhs} ^ {rhs};"); emit_flags_from_logic(out, "value", &carry); }
                ArmDataOp::Add => { let _ = writeln!(out, "    rt.add({rd}, {lhs}, {rhs}, {set_flags});"); }
                ArmDataOp::Adc => { let _ = writeln!(out, "    rt.adc({rd}, {lhs}, {rhs}, {set_flags});"); }
                ArmDataOp::Sub => { let _ = writeln!(out, "    rt.sub({rd}, {lhs}, {rhs}, {set_flags});"); }
                ArmDataOp::Rsb => { let _ = writeln!(out, "    rt.sub({rd}, {rhs}, {lhs}, {set_flags});"); }
                ArmDataOp::Sbc => { let _ = writeln!(out, "    rt.sbc({rd}, {lhs}, {rhs}, {set_flags});"); }
                ArmDataOp::Rsc => { let _ = writeln!(out, "    rt.sbc({rd}, {rhs}, {lhs}, {set_flags});"); }
        ...
        ArmExtended::Multiply { rd, rn, rs, rm, accumulate, set_flags } => {
            let suffix = if accumulate { format!(".wrapping_add(rt.read_reg({rn}))") } else { String::new() };
        ...
        ArmExtended::Swap { rd, rn, rm, byte } => {
            ...
            if byte { let _ = writeln!(out, "    let old = rt.read8(address); rt.write8(address, rt.read_reg({rm}) as u8); rt.write_reg({rd}, old as u32);"); } else { let _ = writeln!(out, "    let old = rt.read32(address); rt.write32(address & !3, rt.read_reg({rm})); rt.write_reg({rd}, old);"); }
        }
        ...
        ArmExtended::SingleDataTransfer { load, byte, rd, rn, offset, pre_index, up, write_back } => {
            ...
            if load { let _ = writeln!(out, "    rt.write_reg({rd}, if {byte} {{ rt.read8(address) as u32 }} else {{ rt.read32(address) }});"); } else if byte { let _ = writeln!(out, "    rt.write8(address, rt.read_reg({rd}) as u8);"); } else { let _ = writeln!(out, "    rt.write32(address & !3, rt.read_reg({rd}));"); }
