...
pub fn emit_thumb_extended(out: &mut String, op: ThumbExtended) {
    match op {
        ...
        ThumbExtended::AddSubImmediate { sub, rd, rs, imm } => {
            if sub { let _ = writeln!(out, "    rt.sub({rd}, rt.read_reg({rs}), {imm}u32, true);"); }
            else { let _ = writeln!(out, "    rt.add({rd}, rt.read_reg({rs}), {imm}u32, true);"); }
        }
        ...
        ThumbExtended::PushPop { load, registers, extra_lr_pc } => {
            ...
            if load {
                ...
                if extra_lr_pc { let _ = writeln!(out, "    let target = rt.read32(address); rt.write_reg(13, rt.read_reg(13).wrapping_add({count} * 4)); return Ok(GeneratedBlockExit::continue_to(target & !1, true));"); }
