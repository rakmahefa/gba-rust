...
fn emit_op(out: &mut String, op: &IrOp) {
    match op {
        ...
        IrOp::Mov { dst, src, set_flags } => {
            ...
            if *set_flags {
                let _ = writeln!(out, "    rt.trace_recompiled(0, 0); // flag-setting MOV is modeled in IR; runtime flag helper pending");
            }
        }
        IrOp::Add { dst, lhs, rhs, set_flags } => {
            ...
            if *set_flags {
                let _ = writeln!(out, "    rt.trace_recompiled(0, 0); // flag-setting ADD is modeled in IR; runtime flag helper pending");
            }
        }
        IrOp::Sub { dst, lhs, rhs, set_flags } => {
            ...
            if *set_flags {
                let _ = writeln!(out, "    rt.trace_recompiled(0, 0); // flag-setting SUB is modeled in IR; runtime flag helper pending");
            }
        }
        IrOp::Cmp { lhs, rhs } => {
        ...
        IrOp::ArmExtended { .. } => {
            emit_unimplemented(out, 0, 0, "ArmExtended");
        }
        IrOp::ThumbExtended { .. } => {
            emit_unimplemented(out, 0, 0, "ThumbExtended");
        }
        IrOp::Unknown { address, raw, mode } => {
            ...
            emit_unimplemented(out, *address, *raw, mode);
        }
    }
}
...
pub fn generate(program: &Program, module_name: &str) -> RustModule {
    ...
    for block in &program.cfg.blocks {
        ...
        for ins in &block.ir {
            for op in &ins.ops { emit_op(&mut out, op); }
        }
