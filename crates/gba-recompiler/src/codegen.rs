...
use crate::decoder::{
    ArmDataOp, ArmExtended, Condition, Mode, Operand2, ThumbAluOp, ThumbExtended,
};
    ...
    match op {
        ...
        ThumbExtended::AddSubRegister { sub, rd, rs, rn } => {
            if sub {
                let _ = writeln!(
                    out,
                    "    rt.sub({rd}, rt.read_reg({rs}), rt.read_reg({rn}), true);"
                );
            } else {
                let _ = writeln!(
                    out,
                    "    rt.add({rd}, rt.read_reg({rs}), rt.read_reg({rn}), true);"
                );
            }
        }
        ...
        ThumbExtended::Alu { op, rd, rs } => {
            ...
            match op {
                ThumbAluOp::And => {
                    ...
                    emit_flags_from_logic(out, "value", "None");
                }
                ThumbAluOp::Eor => {
                    let _ = writeln!(
                        out,
                ...
                ThumbAluOp::Neg => {
                    let _ = writeln!(out, "    rt.sub({rd}, 0, {rhs}, true);");
                }
                ThumbAluOp::Adc => {
                    let _ = writeln!(out, "    rt.adc({rd}, {lhs}, {rhs}, true);");
                }
                ThumbAluOp::Sbc => {
                    let _ = writeln!(out, "    rt.sbc({rd}, {lhs}, {rhs}, true);");
                }
                ...
                ThumbAluOp::Mul => {
                    let _ = writeln!(
                        out,
                        "    let value = {lhs}.wrapping_mul({rhs}); rt.write_reg({rd}, value);"
                    );
                    emit_flags_from_logic(out, "value", "None");
                }
            }
        ...
        ThumbExtended::HighRegister { op, rd, rs } => match op {
            0 => {
                let _ = writeln!(
                    out,
                    "    rt.add({rd}, rt.read_reg({rd}), rt.read_reg({rs}), false);"
                );
            }
            1 => emit_cmp_sub(
                out,
                &format!("rt.read_reg({rd})"),
                &format!("rt.read_reg({rs})"),
            ),
            2 => {
                let _ = writeln!(out, "    rt.mov({rd}, rt.read_reg({rs}), false);");
            }
            _ => {
                let _ = writeln!(out, "    return Err(\"unsupported high-register Thumb operation in specialized codegen\");");
            }
        },
        ...
        ThumbExtended::LoadStoreRegister {
            ...
            let _ = writeln!(
                out,
                "    let address = rt.read_reg({rb}).wrapping_add(rt.read_reg({ro}));"
            );
            if load {
                let _ = writeln!(out, "    rt.write_reg({rd}, if {byte} {{ rt.read8(address) as u32 }} else {{ rt.read32(address) }});");
            } else if byte {
        ...
        ThumbExtended::LoadStoreSignHalf { kind, rd, rb, ro } => {
            ...
            match kind {
                ...
                1 => {
                    let _ = writeln!(out, "    let value = rt.read8(address) as u32; rt.write_reg({rd}, if value & 0x80 != 0 {{ value | 0xffff_ff00 }} else {{ value }});");
                }
                2 => {
                    let _ = writeln!(out, "    rt.write_reg({rd}, rt.read16(address) as u32);");
                }
                3 => {
                    let _ = writeln!(out, "    let value = rt.read16(address) as u32; rt.write_reg({rd}, if value & 0x8000 != 0 {{ value | 0xffff_0000 }} else {{ value }});");
                }
                _ => {
                    let _ = writeln!(out, "    return Err(\"unsupported sign/halfword Thumb operation in specialized codegen\");");
                }
            }
        }
        ThumbExtended::LoadStoreImmediate {
            ...
            let _ = writeln!(
                out,
                "    let address = rt.read_reg({rb}).wrapping_add({}u32 * {scale});",
                offset
            );
            if load {
                let _ = writeln!(out, "    rt.write_reg({rd}, if {byte} {{ rt.read8(address) as u32 }} else {{ rt.read32(address) }});");
        ...
        ThumbExtended::LoadStoreHalfword {
            ...
            let _ = writeln!(
                out,
                "    let address = rt.read_reg({rb}).wrapping_add({}u32 * 2);",
                offset
            );
            if load {
                let _ = writeln!(out, "    rt.write_reg({rd}, rt.read16(address) as u32);");
        ...
        ThumbExtended::SpRelativeLoadStore { load, rd, offset } => {
            let _ = writeln!(
                out,
                "    let address = rt.read_reg(13).wrapping_add({}u32 * 4);",
                offset
            );
            if load {
                let _ = writeln!(out, "    rt.write_reg({rd}, rt.read32(address));");
            } else {
                let _ = writeln!(out, "    rt.write32(address & !3, rt.read_reg({rd}));");
            }
        }
        ...
        ThumbExtended::AddSp { negative, imm } => {
            if negative {
                let _ = writeln!(
                    out,
                    "    rt.write_reg(13, rt.read_reg(13).wrapping_sub({imm}u32));"
                );
            } else {
                let _ = writeln!(
                    out,
                    "    rt.write_reg(13, rt.read_reg(13).wrapping_add({imm}u32));"
                );
            }
        }
        ThumbExtended::PushPop {
            ...
            if load {
                ...
                if extra_lr_pc {
                    ...
                    let _ = writeln!(
                        out,
                        "    rt.write_reg(13, rt.read_reg(13).wrapping_add({count} * 4));"
                    );
                }
            } else {
                let _ = writeln!(
                ...
                let _ = writeln!(
                    out,
                    "    rt.write_reg(13, rt.read_reg(13).wrapping_sub({count} * 4));"
                );
            }
        }
...
fn emit_linked_predicate(out: &mut String, semantic: &SemanticProgram) {
    let _ = writeln!(
        out,
        "fn is_linked_block(address: u32, thumb: bool) -> bool {{"
    );
