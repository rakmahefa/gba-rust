...
fn condition_code(condition: Condition) -> u8 {
    match condition {
        Condition::Eq => 0x0, Condition::Ne => 0x1, Condition::Cs => 0x2, Condition::Cc => 0x3,
        Condition::Mi => 0x4, Condition::Pl => 0x5, Condition::Vs => 0x6, Condition::Vc => 0x7,
        Condition::Hi => 0x8, Condition::Ls => 0x9, Condition::Ge => 0xA, Condition::Lt => 0xB,
        Condition::Gt => 0xC, Condition::Le => 0xD, Condition::Al => 0xE,
    }
}
...
fn emit_thumb_extended(out: &mut String, op: ThumbExtended) {
    match op {
        ...
        ThumbExtended::PcRelativeLoad { rd, word_offset } => { let _ = writeln!(out, "    let address = (rt.read_reg(15) & !3).wrapping_add({}u32 * 4); rt.write_reg({rd}, rt.read32(address));", word_offset); }
        ...
        ThumbExtended::SoftwareInterrupt { .. } => { let _ = writeln!(out, "    let (target, thumb) = rt.raise_exception(gba_runtime::ExceptionKind::SoftwareInterrupt); return Ok(GeneratedBlockExit::continue_to(target, thumb));"); }
    }
}
...
fn emit_terminator(out: &mut String, block: &SemanticBlock, program: &Program) {
    let (address, size) = block.instructions.last().map(|instruction| (instruction.address, instruction.size)).unwrap_or((block.address, 0));
...
fn emit_block(out: &mut String, program: &Program, semantic: &SemanticProgram, block_id: BlockId) {
    let block = semantic.functions.iter().flat_map(|function| function.blocks.iter()).find(|block| block.id == block_id).unwrap_or_else(|| panic!("semantic block {block_id:?} missing"));
