use crate::decoder::{ArmDataOp, ArmExtended, ArmOp, Instruction, InstructionKind, Operand2, ThumbExtended, ThumbOp, Mode};
...
pub(super) fn operand_value(state: AbstractState, operand: Operand2) -> AbstractValue {
    match operand {
...
pub(super) fn add_values(lhs: AbstractValue, rhs: AbstractValue) -> AbstractValue {
    match (lhs, rhs) {
...
pub(super) fn sub_values(lhs: AbstractValue, rhs: AbstractValue) -> AbstractValue {
    match (lhs, rhs) {
...
pub(super) fn transfer_instruction(
    ...
    match instruction.kind {
        ...
        InstructionKind::Thumb(ThumbOp::Extended(ThumbExtended::AddSubImmediate { sub, rd, rs, imm })) => {
            let value = if sub { sub_values(state.read(rs), AbstractValue::Constant(imm as u32)) } else { add_values(state.read(rs), AbstractValue::Constant(imm as u32)) };
            state.write(rd, value);
        }
        InstructionKind::Thumb(ThumbOp::Extended(ThumbExtended::AddSubRegister { sub, rd, rs, rn })) => {
            let value = if sub { sub_values(state.read(rs), state.read(rn)) } else { add_values(state.read(rs), state.read(rn)) };
            state.write(rd, value);
        }
        ...
        InstructionKind::Arm(ArmOp::Extended(ArmExtended::SingleDataTransfer { load: true, rd, rn: 15, offset: Operand2::Imm(offset), pre_index: true, up, write_back: false, .. })) => {
...
pub(super) fn resolved_exchange_target(state: AbstractState, register: u8) -> Option<BlockKey> {
...
fn in_rom(rom: &[u8], address: u32) -> bool {
    address >= crate::decoder::ROM_BASE && address - crate::decoder::ROM_BASE < rom.len() as u32
}

fn read_rom32(rom: &[u8], address: u32) -> Option<u32> {
    let base = crate::decoder::ROM_BASE;
    if !in_rom(rom, address) || address - base > rom.len().saturating_sub(4) as u32 { return None; }
