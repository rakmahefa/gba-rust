...
pub enum ValidationError {
    #[error("CFG is empty")]
    EmptyGraph,
    #[error("CFG entry {0:?} is invalid")]
    InvalidEntry(BlockId),
    #[error("block id is not contiguous: expected {expected:?}, got {actual:?}")]
    ...
    EmptyBlock(BlockId),
    #[error("block {block:?} key mismatch: expected {expected:?}, got {actual:?}")]
    BlockKeyMismatch { block: BlockId, expected: BlockKey, actual: BlockKey },
    #[error("duplicate instruction at {address:#x} in {mode:?}")]
    DuplicateInstruction { address: u32, mode: Mode },
    #[error("block {block:?} instructions are not contiguous: {previous:#x} -> {next:#x}")]
    NonContiguousInstructions { block: BlockId, previous: u32, next: u32 },
    #[error("block {block:?} changes execution mode from {previous:?} to {next:?}")]
    ModeChangedWithinBlock { block: BlockId, previous: Mode, next: Mode },
    #[error("block {block:?} has invalid successor {successor:?}")]
    ...
    #[error("block {block:?} has {instructions} instructions but {ir} IR instructions")]
    InstructionIrMismatch { block: BlockId, instructions: usize, ir: usize },
    #[error("instruction count mismatch: expected {expected}, got {actual}")]
    InstructionCountMismatch { expected: usize, actual: usize },
    #[error("block {0:?} is unreachable from the CFG entry")]
    UnreachableBlock(BlockId),
    #[error("block {block:?} is missing edge to {target:?}")]
    MissingEdge { block: BlockId, target: BlockKey },
    #[error("block {block:?} has unexpected edge to {target:?}")]
    UnexpectedEdge { block: BlockId, target: BlockKey },
}

fn block_keys(cfg: &ControlFlowGraph) -> HashMap<BlockId, BlockKey> {
    cfg.blocks.iter().map(|block| (block.id, block.key.clone())).collect()
}

fn successor_keys(block: &BasicBlock, block_keys: &HashMap<BlockId, BlockKey>) -> HashSet<BlockKey> {
    block.successors.iter().filter_map(|id| block_keys.get(id).cloned()).collect()
}
...
fn require_edge(
    ...
    if owners.contains_key(target) && !successor_keys(block, keys).contains(target) {
        return Err(ValidationError::MissingEdge { block: block.id, target: target.clone() });
    }
    Ok(())
}
...
fn validate_terminator_edges(
    ...
    let Some(last) = block.instructions.last().copied() else { return Ok(()); };
    ...
    match last.kind {
        ...
        InstructionKind::Thumb(ThumbOp::Branch { target, condition }) => {
            let target = BlockKey { address: target, mode: Mode::Thumb };
            let mut allowed = HashSet::new();
            if owners.contains_key(&target) { allowed.insert(target.clone()); require_edge(block, &target, owners, keys)?; }
            if condition != Condition::Al && owners.contains_key(&next) {
                allowed.insert(next.clone()); require_edge(block, &next, owners, keys)?;
            }
            if let Some(unexpected) = actual.iter().find(|key| !allowed.contains(*key)) {
                return Err(ValidationError::UnexpectedEdge { block: block.id, target: unexpected.clone() });
            }
        ...
        InstructionKind::Arm(ArmOp::BranchExchange { link, .. }) => {
            if actual.len() > usize::from(link) + 1 {
                return Err(ValidationError::UnexpectedEdge { block: block.id, target: next });
            }
            if link && owners.contains_key(&next) && !actual.contains(&next) {
                return Err(ValidationError::MissingEdge { block: block.id, target: next });
            }
        }
        ...
        InstructionKind::Arm(ArmOp::Unknown) | InstructionKind::Thumb(ThumbOp::Unknown) => {
            if let Some(unexpected) = actual.into_iter().next() {
                return Err(ValidationError::UnexpectedEdge { block: block.id, target: unexpected });
            }
        }
        _ => {
            require_edge(block, &next, owners, keys)?;
            if let Some(unexpected) = actual.iter().find(|key| *key != &next) {
                return Err(ValidationError::UnexpectedEdge { block: block.id, target: unexpected.clone() });
            }
        }
    }
...
fn validate_block(
    ...
    if block.instructions.is_empty() { return Err(ValidationError::EmptyBlock(block.id)); }
    ...
    for successor in &block.successors {
        if successor.0 >= block_count { return Err(ValidationError::InvalidSuccessor { block: block.id, successor: *successor }); }
        if !successors.insert(*successor) { return Err(ValidationError::DuplicateSuccessor { block: block.id, successor: *successor }); }
    }
    Ok(())
}
...
pub(super) fn validate_cfg(
    ...
    if cfg.blocks.is_empty() { return Err(ValidationError::EmptyGraph); }
    if cfg.entry.0 >= cfg.blocks.len() { return Err(ValidationError::InvalidEntry(cfg.entry)); }
    ...
    for (index, block) in cfg.blocks.iter().enumerate() {
        ...
        if block.id != expected_id { return Err(ValidationError::NonContiguousBlockId { expected: expected_id, actual: block.id }); }
        if !seen.insert(block.key.clone()) { return Err(ValidationError::DuplicateBlockKey(block.key.clone())); }
        validate_block(block, cfg.blocks.len(), &mut instruction_owners)?;
    }
    if instruction_owners.len() != expected_instruction_count {
        return Err(ValidationError::InstructionCountMismatch { expected: expected_instruction_count, actual: instruction_owners.len() });
    }
    validate_reachability(cfg)?;
    let keys = block_keys(cfg);
    for block in &cfg.blocks { validate_terminator_edges(block, &instruction_owners, &keys)?; }
    Ok(())
}
