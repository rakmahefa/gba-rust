...
fn validate_instruction(source: &IrInstruction, semantic: &SemanticInstruction, block_id: BlockId) -> Result<(), String> {
...
fn validate_block_against_source(program: &Program, block: &SemanticBlock) -> Result<(), String> {
    let source = program.cfg.blocks.get(block.id.0).ok_or_else(|| format!("semantic block {} does not exist in source CFG", block.id.0))?;
    ...
    if source.instructions.len() != block.instructions.len() {
        return Err(format!("block {} instruction count changed during semantic lowering", block.id.0));
    }
...
fn validate_successors(program: &Program, block: &SemanticBlock) -> Result<(), String> {
    ...
    let expected = match &block.terminator {
        ...
        _ => program.cfg.blocks.get(block.id.0).map(|source| source.successors.clone()).ok_or_else(|| format!("block {} does not exist in source CFG", block.id.0))?,
    };
    if block.successors != expected {
        return Err(format!("block {} semantic successors differ from source control-flow contract", block.id.0));
    }
    Ok(())
}

fn validate_terminator(program: &Program, block: &SemanticBlock) -> Result<(), String> {
    let terminal_effect = block.instructions.last().map(SemanticInstruction::control_effect).unwrap_or(IrControlEffect::None);
    match (&block.terminator, terminal_effect) {
        (SemanticTerminator::Fallthrough, IrControlEffect::None) => {}
        (SemanticTerminator::Branch { target, condition }, IrControlEffect::Branch { target: effect_target, condition: effect_condition, link: false }) if *target == effect_target && *condition == effect_condition => {
        ...
        (SemanticTerminator::Call { target, condition }, IrControlEffect::Branch { target: effect_target, condition: effect_condition, link: true }) if *target == effect_target && *condition == effect_condition => {
        ...
        (SemanticTerminator::IndirectCall { register, mode }, IrControlEffect::BranchExchange { register: effect_register, link: true }) if *register == effect_register && *mode == block.mode => {
            if block.successors.is_empty() {
                return Err(format!("indirect call block {} lost its continuation", block.id.0));
            }
        }
        (SemanticTerminator::IndirectBranch { register }, IrControlEffect::BranchExchange { register: effect_register, link: false }) if *register == effect_register => {}
...
fn validate_function_metadata(program: &Program, functions: &FunctionControlFlowGraph, semantic: &SemanticProgram) -> Result<(), String> {
    ...
    for (index, function) in semantic.functions.iter().enumerate() {
        if function.id.0 != index {
            return Err(format!("semantic function id {} is not at vector index {}", function.id.0, index));
        }
        ...
        if function.blocks.iter().all(|block| block.id != function.entry) {
            return Err(format!("function {} does not contain its entry block {}", function.id.0, function.entry.0));
        }
        for successor in &function.successors {
            if successor.0 >= semantic.functions.len() {
                return Err(format!("function {} has invalid function successor {}", function.id.0, successor.0));
            }
        }
    }
    Ok(())
...
pub fn validate_semantic_program(program: &Program, functions: &FunctionControlFlowGraph, semantic: &SemanticProgram) -> Result<(), String> {
    ...
    for call in semantic.functions.iter().flat_map(|function| function.calls.iter()) {
        ...
        if call.instruction_index >= program.cfg.blocks[call.block.0].ir.len() {
            return Err(format!("call site references invalid instruction {} in block {}", call.instruction_index, call.block.0));
        }
        ...
        for return_site in &call.return_sites {
            if return_site.block.0 >= program.cfg.blocks.len() || return_site.instruction_index >= program.cfg.blocks[return_site.block.0].instructions.len() {
                return Err(format!("call site references invalid return site {}:{}", return_site.block.0, return_site.instruction_index));
            }
        }
    }
    for return_site in semantic.functions.iter().flat_map(|function| function.returns.iter()) {
        if return_site.block.0 >= program.cfg.blocks.len() || return_site.instruction_index >= program.cfg.blocks[return_site.block.0].instructions.len() {
            return Err(format!("return site references invalid instruction {}:{}", return_site.block.0, return_site.instruction_index));
        }
    }
    Ok(())
}
