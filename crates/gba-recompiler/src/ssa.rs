...
fn merge_state(
    ...
    for reg in 0..16u8 {
        let incoming: Vec<ValueId> = preds
            .iter()
            .filter_map(|pred| out_states.get(pred).and_then(|s| s[reg as usize]))
            .collect();
        ...
        if incoming.iter().all(|value| *value == incoming[0]) {
            ...
            let value = *phis.entry((block.id, reg)).or_insert_with(|| {
                allocator.fresh(ValueDef::Phi { block: block.id, register: reg })
            });
...
fn lower_operand(value: &Value, state: &RegisterState) -> SsaValue {
    match value {
        ...
        Value::Reg(reg) => SsaValue::Value(
            state[*reg as usize].expect("register use must have an SSA value"),
        ),
    }
}
...
fn lower_instruction(
    ...
    definitions: &InstructionDefinitions,
) -> SsaInstruction {
    let definition = |register: u8| {
        *definitions
    ...
    let ssa = match op {
        ...
        IrOp::Mov { dst, src } => {
            ...
            let value = definition(*dst);
            state[*dst as usize] = Some(value);
