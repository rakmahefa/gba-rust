...
impl IrFlags {
    pub fn condition_read(c: Condition) -> Self {
        match c {
            ...
            Condition::Eq | Condition::Ne => Self {
                read_z: true,
                ..Self::default()
            },
            Condition::Cs | Condition::Cc => Self {
                read_c: true,
                ..Self::default()
            },
            Condition::Mi | Condition::Pl => Self {
                read_n: true,
                ..Self::default()
            },
            Condition::Vs | Condition::Vc => Self {
                read_v: true,
                ..Self::default()
            },
            Condition::Hi | Condition::Ls => Self {
                read_c: true,
                read_z: true,
                ..Self::default()
            },
            Condition::Ge | Condition::Lt => Self {
                read_n: true,
                read_v: true,
                ..Self::default()
            },
    ...
    pub fn arithmetic_write(set: bool) -> Self {
        if set {
            Self {
                write_n: true,
                write_z: true,
    ...
    pub fn logical_write(set: bool) -> Self {
        if set {
            Self {
                write_n: true,
                write_z: true,
...
fn add_unique(v: &mut Vec<u8>, r: u8) {
    if !v.contains(&r) {
        v.push(r);
    }
}
fn operand_reads(v: &mut Vec<u8>, op: Operand2) {
    if let Operand2::Reg {
        rm,
        by_register,
        shift_register,
    ...
    } = op
    {
        add_unique(v, rm);
        if by_register {
            add_unique(v, shift_register);
        }
    }
}
fn reg_list(v: &mut Vec<u8>, list: u16) {
    for r in 0..16 {
        if list & (1 << r) != 0 {
            add_unique(v, r as u8);
        }
    }
}
fn reg_list8(v: &mut Vec<u8>, list: u8) {
    for r in 0..8 {
        if list & (1 << r) != 0 {
            add_unique(v, r as u8);
        }
...
fn arm_reads(op: ArmExtended) -> Vec<u8> {
    ...
    match op {
        ...
        ArmExtended::MultiplyLong {
            ...
            add_unique(&mut v, rs);
            if accumulate {
                add_unique(&mut v, rd_lo);
                add_unique(&mut v, rd_hi);
            }
        }
        ...
        ArmExtended::BlockTransfer {
            ...
            add_unique(&mut v, rn);
            if !load {
                reg_list(&mut v, register_list);
            }
        }
        ArmExtended::Mrs { .. } => {}
        ArmExtended::Msr { source, .. } => operand_reads(&mut v, source),
        ArmExtended::SoftwareInterrupt { .. } => {}
        ArmExtended::CoprocessorTransfer { crn, crd, .. } => {
            add_unique(&mut v, crn);
            add_unique(&mut v, crd);
        }
...
fn arm_writes(op: ArmExtended) -> Vec<u8> {
    ...
    match op {
        ...
        ArmExtended::CoprocessorRegisterTransfer { to_arm, rd, .. } => {
            if to_arm {
                v.push(rd)
            }
        }
...
fn arm_flags(op: ArmExtended) -> IrFlags {
    match op {
        ArmExtended::DataProcessing { op, set_flags, .. } => match op {
            ...
            | ArmDataOp::Bic
            | ArmDataOp::Mov
            | ArmDataOp::Mvn => IrFlags::logical_write(set_flags),
            ArmDataOp::Sub
            | ArmDataOp::Rsb
            | ArmDataOp::Add
            | ArmDataOp::Adc
...
fn arm_memory(op: ArmExtended) -> Option<IrMemoryEffect> {
    match op {
        ...
        ArmExtended::CoprocessorTransfer { crn, load, .. } => Some(IrMemoryEffect {
            kind: if load {
                IrMemoryKind::Read
            } else {
                IrMemoryKind::Write
...
fn thumb_reads(op: ThumbExtended) -> Vec<u8> {
    ...
    match op {
        ...
        ThumbExtended::PushPop {
            ...
            add_unique(&mut v, 13);
            if !load {
                reg_list8(&mut v, registers);
                if extra_lr_pc {
                    add_unique(&mut v, 14);
                }
            }
        }
        ThumbExtended::MultipleLoadStore {
            load,
            rb,
            register_list,
        } => {
...
fn thumb_writes(op: ThumbExtended) -> Vec<u8> {
    ...
    match op {
        ...
        | ThumbExtended::AddSubImmediate { rd, .. } => v.push(rd),
...
fn thumb_flags(op: ThumbExtended) -> IrFlags {
    match op {
        ...
        ThumbExtended::AddSubRegister { .. } | ThumbExtended::AddSubImmediate { .. } => {
            IrFlags::arithmetic_write(true)
        }
        ThumbExtended::Alu { op, .. } => match op {
            ...
            _ => {
                ...
                if matches!(op, ThumbAluOp::Adc | ThumbAluOp::Sbc) {
                    f.read_c = true;
                    f.write_v = true;
                }
                if matches!(op, ThumbAluOp::Neg | ThumbAluOp::Mul) {
                    f.write_v = matches!(op, ThumbAluOp::Neg);
                }
                f
            }
        },
...
fn thumb_memory(op: ThumbExtended) -> Option<IrMemoryEffect> {
    match op {
        ...
        ThumbExtended::SpRelativeLoadStore { load, .. } | ThumbExtended::PushPop { load, .. } => {
            Some(IrMemoryEffect {
                kind: if load {
                    IrMemoryKind::Read
                } else {
...
impl IrOp {
    ...
    pub fn flags(&self) -> IrFlags {
        match self {
            ...
            Self::Add { set_flags, .. } | Self::Sub { set_flags, .. } => {
                IrFlags::arithmetic_write(*set_flags)
            }
...
impl IrInstruction {
    ...
    pub fn with_source_raw(mut self, raw: u32) -> Self {
        self.source_raw = raw;
        self
    }
    pub fn reads(&self) -> Vec<u8> {
        let mut v = Vec::new();
        for r in self.ops.iter().flat_map(IrOp::reads) {
            add_unique(&mut v, r);
        }
        v.sort_unstable();
        v
    ...
    pub fn writes(&self) -> Vec<u8> {
        let mut v = Vec::new();
        for r in self.ops.iter().flat_map(IrOp::writes) {
            add_unique(&mut v, r);
        }
        v.sort_unstable();
        v
    ...
    pub fn flags(&self) -> IrFlags {
        self.ops.iter().fold(IrFlags::default(), |mut a, op| {
            let b = op.flags();
            a.read_n |= b.read_n;
            a.read_z |= b.read_z;
            a.read_c |= b.read_c;
    ...
    pub fn control(&self) -> IrControlEffect {
        self.ops
            ...
            .rev()
            .map(IrOp::control)
            .find(|e| !matches!(e, IrControlEffect::None))
...
pub fn lower(ins: Instruction) -> IrInstruction {
    let op = match ins.kind {
        ...
        InstructionKind::Arm(ArmOp::Mov { rd, op2 }) => IrOp::Mov {
            dst: rd,
            src: operand(op2),
            set_flags: ins.raw & (1 << 20) != 0,
        },
        InstructionKind::Arm(ArmOp::Add { rd, rn, op2 }) => IrOp::Add {
            dst: rd,
            lhs: rn,
            rhs: operand(op2),
            set_flags: ins.raw & (1 << 20) != 0,
        },
        InstructionKind::Arm(ArmOp::Sub { rd, rn, op2 }) => IrOp::Sub {
            dst: rd,
            lhs: rn,
            rhs: operand(op2),
            set_flags: ins.raw & (1 << 20) != 0,
        },
        InstructionKind::Arm(ArmOp::Cmp { rn, op2 }) => IrOp::Cmp {
            lhs: rn,
            rhs: operand(op2),
        },
        InstructionKind::Arm(ArmOp::Load {
            rd,
            rn,
            offset,
            byte,
        ...
        InstructionKind::Arm(ArmOp::Store {
            rd,
            rn,
            offset,
            byte,
        ...
        InstructionKind::Arm(ArmOp::BranchExchange { rm, link }) => {
            IrOp::BranchExchange { register: rm, link }
        }
        InstructionKind::Arm(ArmOp::Extended(op)) => IrOp::ArmExtended { op },
        InstructionKind::Arm(ArmOp::Unknown) => IrOp::Unknown {
            address: ins.address,
            raw: ins.raw,
            mode: ins.mode,
        },
        InstructionKind::Thumb(ThumbOp::MovImm { rd, imm }) => IrOp::Mov {
            dst: rd,
        ...
        InstructionKind::Thumb(ThumbOp::SubImm { rd, rn, imm }) => IrOp::Sub {
            dst: rd,
            lhs: rn,
            rhs: Value::Imm(imm as u32),
            set_flags: true,
        },
        InstructionKind::Thumb(ThumbOp::LoadImm {
            rd,
            rn,
            word_offset,
        }) => IrOp::Load {
        ...
        InstructionKind::Thumb(ThumbOp::StoreImm {
            rd,
            rn,
            word_offset,
        }) => IrOp::Store {
        ...
        InstructionKind::Thumb(ThumbOp::Branch { target, condition }) => IrOp::Branch {
            target,
            condition,
            link: false,
        },
        InstructionKind::Thumb(ThumbOp::BranchLink { target }) => IrOp::Branch {
            target,
            condition: Condition::Al,
            link: true,
        },
        InstructionKind::Thumb(ThumbOp::BranchExchange { rm }) => IrOp::BranchExchange {
            register: rm,
            link: false,
        },
        InstructionKind::Thumb(ThumbOp::Extended(op)) => IrOp::ThumbExtended { op },
        InstructionKind::Thumb(ThumbOp::Unknown) => IrOp::Unknown {
            address: ins.address,
            raw: ins.raw,
            mode: ins.mode,
        },
